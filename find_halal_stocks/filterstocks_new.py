...
HALAL_CASH_PERC = 30.0 # From AAOIFI Standards
HALAL_DEBT_PERC = 30.0 # From AAOIFI Standards
HALAL_RECEIVABLES_PERC = 49.0 # DEPRECATED

LIST_OF_BAD_INDUSTRIES = ['Healthcare Plans']

LIST_OF_BAD_WORDS = ['cannabis', 'mortgage', 'bonds', 'fixed income', 'insurance', 'alcohol', 'music',
                     'loan', 'lending', 'pork']
...
# NOTE: Really should be including interest bearing deposits here as well, but
#       the balance_sheet from yahoo_fin just lumps it with liabilities.
#       However, I have only seen deposits with banks, which are haram anyways
...
# Uses widely regarded AAOIFI standards as the basis for shariah compliance
# AAOIFI, according to Zoya finance, does now not screen for receivables, which is reflected below
def Filter_By_AAOIFI_Standards(tickers):
   ...
   #return recv_check
# End Filter_By_AAOIFI_Standards

# Takes the description of the company from Yahoo Finance and looks for indications of
# haram business, such as "cannabis" or "mortgages"
# Does a basic check for industry type first
def Has_Haram_Business(ticker):
   url = "https://finance.yahoo.com/quote/" + ticker + "/profile?p=" + ticker

   page = requests.get(url, headers)
   tree = html.fromstring(page.content)

   industry_list = tree.xpath("//p[span='Industry']/span[position()=4]/text()")
   if (industry_list):
      industry = industry_list[0]
      if (any(bad_industry in industry for bad_industry in LIST_OF_BAD_INDUSTRIES)):
         return True

   description_list = tree.xpath("//h2[span='Description']/../p/text()")
   if (description_list):
      description = description_list[0]
   else:
      return False

   # Look for at least 4 occurrences total to flag as bad. For example, some tax software companies
   # mention mortage in their profile but I would not consider that haram as they may provide tax services
   # to members of the mortgage industry. Or like Tesla which mentions insurance in their profile, which
   # is arguably okay
   total_occurrences = 0
   for bad_word in LIST_OF_BAD_WORDS:
      total_occurrences += description.count(bad_word)
      if (total_occurrences >= 4):
         return True
   return False
# End Filter_By_Core_Business
...
# First, filter by financials
halal_tickers = Filter_By_AAOIFI_Standards(all_tickers_df)
print(halal_tickers)

# Next, do it based on company's core businesses
halal_tickers_list = list(halal_tickers.index)
print("HARAM BUSINESSES")
for ticker in halal_tickers_list:
   if (Has_Haram_Business(ticker)):
      print(ticker)
      halal_tickers = halal_tickers.drop(ticker)
